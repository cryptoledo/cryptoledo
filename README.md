<div align="center">

# JToledo

### Founder · AI Sync System Inc. · Building the Conscience for AI Agents


[![NVIDIA Inception](https://img.shields.io/badge/NVIDIA_Inception-Member_2026-76b900?style=for-the-badge&logo=nvidia&logoColor=white)](https://nvidia.com/startups)
[![Patent Pending](https://img.shields.io/badge/USPTO_%2364%2F065%2C108-Patent_Pending-0066cc?style=for-the-badge&logo=gov.uk&logoColor=white)](https://malse.dev)
[![Lovable Diamond](https://img.shields.io/badge/Lovable-Diamond_Builder-b9f2ff?style=for-the-badge&logo=diamond&logoColor=000000)](https://lovable.dev)

[![AWS Activate](https://img.shields.io/badge/AWS_Activate-$5K_Credits-FF9900?style=flat-square&logo=amazonaws&logoColor=white)](https://aws.amazon.com/activate)
[![Google for Startups](https://img.shields.io/badge/Google_for_Startups-Cloud_Program-4285F4?style=flat-square&logo=google&logoColor=white)](https://cloud.google.com/startup)
[![Microsoft for Startups](https://img.shields.io/badge/Microsoft_for_Startups-$5K_Azure_Credits-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)](https://startups.microsoft.com)
[![Notion for Startups](https://img.shields.io/badge/Notion_for_Startups-Unlimited_AI-000000?style=flat-square&logo=notion&logoColor=white)](https://notion.so/startups)

[![Gemini Certified](https://img.shields.io/badge/Google-Gemini_Certified_2025-4285F4?style=flat-square&logo=google&logoColor=white)](https://cloud.google.com)
[![Anthropic Academy](https://img.shields.io/badge/Anthropic_Academy-Claude_101_%2B_Claude_Code_101-cc785c?style=flat-square&logo=anthropic&logoColor=white)](https://anthropic.com)
[![Databricks](https://img.shields.io/badge/Databricks-GenAI_Fundamentals_2024-FF3621?style=flat-square&logo=databricks&logoColor=white)](https://databricks.com)

</div>

---

## What I'm Building

**COGNIS·NEXUS** — cognitive middleware that gives AI agents self-awareness, long-term memory, judgment, and a conscience before they act.

Every AI agent today is powerful but unaccountable — it acts immediately, forgets everything, and can't explain itself. I built the layer that sits between an agent and the world: scoring confidence, running an internal critic, storing episodic memory, and checking policy before any tool call executes. Every action is logged, signed, and auditable.

Built on the **MALSE™ architecture** (patent pending) — five cognitive phases mapped from human brain function to AI agent software:

| Phase | Human Analogy | What It Does |
|-------|--------------|--------------|
| Metacognition | Self-awareness | Scores confidence + hallucination risk per output |
| Attention (Critic) | Devil's advocate | Challenges every uncertain answer before delivery |
| Learning (Memory) | Long-term memory | Remembers every session, every mistake, every win |
| Social Cognition (ToM) | Social awareness | Agents model what other agents know and need |
| Executive Function (CGA) | Conscience | 7-step check before every consequential action |

---

## Projects

<table>
<tr>
<td width="50%" valign="top">

### 🧠 [COGNIS·NEXUS](https://malse.dev)
**Cognitive AI Infrastructure · v1.0.0**

The reference runtime for the Governed Intelligence Protocol — pre-execution governance for AI agents.

- 75+ REST endpoints · A2A v1.2 · MCP-compatible
- Cognitive Guardrail Architecture (S1–S5 escalation)
- Episodic memory · Adversarial critic · Dream State
- Rate limiting · Immutable audit trail
- Patent pending · NVIDIA Inception Member

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white)
![Google Cloud](https://img.shields.io/badge/Google_Cloud-4285F4?style=flat-square&logo=googlecloud&logoColor=white)

</td>
<td width="50%" valign="top">

### ⚡ [AI Sync](https://ai-sync.app)
**Multi-Agent Business Automation · Founding Member**

Four AI agents — Sales, Marketing, Support, Operations — running in parallel with shared memory and human-in-the-loop approval for every consequential action.

- SyncLearn™ — shared cross-agent memory
- SyncSkills™ — structured business outputs
- Governed by COGNIS·NEXUS
- Powered by Google Gemini 2.5

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Gemini](https://img.shields.io/badge/Gemini_2.5-4285F4?style=flat-square&logo=google&logoColor=white)
![Lovable](https://img.shields.io/badge/Built_with_Lovable-b9f2ff?style=flat-square&logoColor=000)

</td>
</tr>
</table>

---

## Startup Programs

> Accepted into every major cloud and AI startup program — multi-cloud from day one.

| Program | Benefit | Status |
|---------|---------|--------|
| **a16z Speedrun** | Founder cohort · Andreessen Horowitz | Applied 2026 |
| **NVIDIA Inception** | GPU credits · Capital Connect · NIM access | ✅ Active 2026 |
| **Google for Startups** | Cloud Program · Approved | ✅ Active 2026 |
| **AWS Activate** | $5,000 in AWS credits (expires Apr 2027) | ✅ Active |
| **Microsoft for Startups** | $5,000 in Azure credits + Azure OpenAI + technical guidance | ✅ Active |
| **Notion for Startups** | Unlimited Notion AI + Notion Plus | ✅ Active |

**Combined cloud compute runway: $10K+ confirmed · Google Cloud TBD · NVIDIA GPU credits up to $162K**

---

## Stack

**AI / Inference**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![Google Gemini](https://img.shields.io/badge/Gemini_2.5-4285F4?style=flat-square&logo=google&logoColor=white)
![Claude](https://img.shields.io/badge/Claude_API-cc785c?style=flat-square&logo=anthropic&logoColor=white)
![NVIDIA NIM](https://img.shields.io/badge/NVIDIA_NIM-76b900?style=flat-square&logo=nvidia&logoColor=white)

**Infrastructure**
![Google Cloud](https://img.shields.io/badge/Google_Cloud_Run-4285F4?style=flat-square&logo=googlecloud&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-FF9900?style=flat-square&logo=amazonaws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)

**Protocols**
![MCP](https://img.shields.io/badge/MCP-Anthropic-cc785c?style=flat-square)
![A2A](https://img.shields.io/badge/A2A_v1.2-Google-4285F4?style=flat-square)
![REST](https://img.shields.io/badge/REST-75%2B_endpoints-009688?style=flat-square)

**Vibe Coding**
![Lovable](https://img.shields.io/badge/Lovable-Diamond_Level-b9f2ff?style=flat-square&logoColor=000)
![Claude Code](https://img.shields.io/badge/Claude_Code-cc785c?style=flat-square&logo=anthropic&logoColor=white)

---

## Credentials

| Program | Credential | Year |
|---------|-----------|------|
| **NVIDIA** | Inception Member | 2026 |
| **Google for Startups** | Cloud Program — Approved | 2026 |
| **AWS** | Activate — $5K Credits | 2026 |
| **Microsoft for Startups** | Founders Hub — $5K Azure Credits | 2025 |
| **Anthropic Academy** | Claude 101 · Claude Code 101 | 2026 |
| **Google** | Gemini Certified (expires 2028) | 2025 |
| **Google Cloud** | Intro to Generative AI · Intro to Image Generation | 2024 |
| **Databricks** | GenAI Fundamentals (expires 2026) | 2024 |
| **Lovable** | Diamond Builder | 2026 |

*Patent pending: USPTO #64/065,108 — MALSE™ architecture & Cognitive Upgrade Protocol*

---

## Currently

- 🔨 Building: NIM adapter for COGNIS·NEXUS (NVIDIA-accelerated governance)
- 🔍 Looking for: Design partners running AI agents at scale who need accountability
- 📍 Based in: British Columbia, Canada

---

<div align="center">

**[malse.dev](https://malse.dev)** · **[ai-sync.app](https://ai-sync.app)** · **[legal@ai-sync.app](mailto:legal@ai-sync.app)**

*"NVIDIA accelerates AI agents. COGNIS·NEXUS makes autonomous execution accountable."*

</div>
